...
const DELIVERY_ACTIONS = [
  ...
  { status: "COMPLETED",             label: "Delivered",         tone: "bg-emerald-600 hover:bg-emerald-700 text-white" },
  { status: "CANCELLED_BY_DELIVERY", label: "Cancel / Issue",    tone: "border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-700" },
];
...
export default function UpcomingTaskCard({ delivery, note, onNoteChange, onUpdateStatus, actionLoading, onTrackMap }) {
  ...
  return (
    <article className="flex flex-col rounded-xl border border-slate-200 bg-white p-5">
      ...
      {/* Info Block */}
      <div className="flex-1 rounded-lg border border-slate-200 bg-slate-50 p-4 space-y-3">
        <div className="flex justify-between">
          <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Phone</span>
          <span className="text-sm font-medium text-slate-700">{delivery?.customerContactNumber || "N/A"}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Assigned</span>
          <span className="text-sm font-medium text-slate-700">{formatDate(delivery?.assignedAt)}</span>
        </div>
          ...
          {delivery?.deliveryLocation?.latitude && delivery?.deliveryLocation?.longitude && (
            <>
              ...
              <button
                ...
                className="mt-1 flex w-full items-center justify-center gap-2 rounded-lg bg-[#1d4ed8] px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-[#1e40af]"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {delivery?.notes && (
          <div className="border-t border-slate-200 pt-3">
            ...
            <p className="mt-1 text-sm italic text-slate-600">"{delivery.notes}"</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 space-y-3 border-t border-slate-100 pt-4">
        <div className="flex items-center gap-2">
          <span className="shrink-0 text-xs font-semibold uppercase tracking-wider text-slate-400">Note</span>
          <input
            type="text"
            value={note || ""}
            onChange={(e) => onNoteChange(id, e.target.value)}
            placeholder="Tracking update..."
            className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700 outline-none transition-colors focus:border-[#1d4ed8]"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {DELIVERY_ACTIONS.map((action) => (
            <button
              ...
              className={`rounded-lg px-2 py-2 text-xs font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${action.tone}`}
            >
