...
import { asCollection, formatMoney, resolveEntityId, resolveRole } from "../../../utils/helpers";
...
export default function StorefrontPage() {
    ...
    const loadProducts = useCallback(async () => {
        ...
        try {
            ...
            setState({
                loading: false,
                error: error?.friendlyMessage || error?.message || "Unable to load products",
                products: [],
            });
        }
    ...
    const handlePlaceOrder = async (event) => {
        ...
        try {
            ...
            setOrderSuccess(`Order ${orderId} placed successfully. +1 loyalty reward point has been applied.`);

            try {
                const me = await AuthService.getCurrentUser();
                authStore.updateUser(me);
            } catch (refreshError) {
                console.warn("Unable to refresh current user after order placement", refreshError);
            }
    ...
    return (
        <div className="relative min-h-screen overflow-x-hidden bg-[#fdf7ec] text-[#1e293b]">
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_0%_0%,_#fde68a_0%,_transparent_35%),radial-gradient(circle_at_100%_20%,_#99f6e4_0%,_transparent_30%),radial-gradient(circle_at_50%_120%,_#c7d2fe_0%,_transparent_40%)]" />

            <header className="sticky top-0 z-20 border-b border-[#f1e6cd] bg-[#fff9ef]/90 backdrop-blur-md">
                <div className="mx-auto flex w-full max-w-[1220px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
                    <Link to="/" className="flex items-center gap-3">
                        <img src={LOGO} alt="Anjana Grocery" className="h-10 w-auto object-contain" />
                        <div>
                            <p className="text-xs uppercase tracking-[0.24em] text-[#475569]">Fresh Market</p>
                            <p className="text-lg font-semibold text-[#0f172a]">Anjana Grocery</p>
                        </div>
                    </Link>

                    <nav className="hidden items-center gap-6 text-sm font-medium text-[#334155] md:flex">
                        <NavLink to="/" end className={({ isActive }) => (isActive ? "text-[#0f172a] underline" : "hover:text-[#0f172a]")}>Home</NavLink>
                        {isUser ? <NavLink to="/my-orders" className="hover:text-[#0f172a]">My Orders</NavLink> : null}
                        {role === "ADMIN" ? <NavLink to="/admin-portal" className="hover:text-[#0f172a]">Admin Portal</NavLink> : null}
                        {role === "DELIVERY" ? <NavLink to="/delivery-portal" className="hover:text-[#0f172a]">Delivery Portal</NavLink> : null}
                    </nav>
                        ...
                        {isLoggedIn ? (
                            <>
                                <span className="hidden rounded-full bg-[#fff1cf] px-3 py-1 text-xs font-semibold text-[#7c2d12] sm:inline-block">
                                    {auth?.user?.name || auth?.user?.contactNumber || "User"} · {role}
                                </span>
                                <button
                                    type="button"
                                    onClick={() => {
                            ...
                            <>
                                <Link to="/login" className="rounded-full border border-[#d1c3a2] bg-white px-4 py-2 text-xs font-semibold text-[#334155] transition hover:bg-[#f8f2e7]">Login</Link>
                                <Link to="/register" className="rounded-full bg-[#0ea5a4] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#0b8e8d]">Register</Link>
                            </>
                        )}
                    </div>
                </div>
            ...
            <main className="relative mx-auto grid w-full max-w-[1220px] gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,1fr)] lg:px-8">
                <section className="space-y-4">
                    <div className="rounded-3xl border border-[#eedfc0] bg-white/90 p-5 shadow-[0_20px_50px_rgba(2,6,23,0.08)]">
                        <p className="text-xs uppercase tracking-[0.2em] text-[#0f766e]">Shop for today</p>
                        <h1 className="mt-2 text-3xl font-semibold leading-tight text-[#0f172a]">Pick groceries, confirm location, and place cash-on-delivery orders.</h1>
                        <p className="mt-3 text-sm text-[#475569]">
                            Products are visible without login. You only need to sign in when you are ready to place an order.
                        </p>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="mt-3 flex items-start justify-between gap-3">
                                            <div>
                                                <h3 className="text-base font-semibold text-[#0f172a]">{product?.name || "Unnamed product"}</h3>
                                                <p className="mt-1 text-xs uppercase tracking-wide text-[#64748b]">{product?.category || "General"}</p>
                                            </div>
                                            <StatusPill status={inStock ? "IN_STOCK" : "OUT_OF_STOCK"} />
                                        </div>

                                        <p className="mt-3 min-h-[42px] text-sm text-[#475569]">{product?.description || "No description"}</p>

                                        <div className="mt-3 flex items-center justify-between">
                                            <p className="text-lg font-semibold text-[#0f766e]">{formatMoney(product?.price)}</p>
                                        ...
                                        <div className="mt-3 flex items-center gap-2">
                                            ...
                                            <span className="min-w-8 text-center text-sm font-semibold text-[#0f172a]">{cartQuantity}</span>
                                            <button
                                                type="button"
                ...
                <aside className="space-y-4 lg:sticky lg:top-24 lg:h-fit">
                    <section className="rounded-3xl border border-[#e4d6b8] bg-white p-5 shadow-[0_20px_50px_rgba(2,6,23,0.08)]">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="rounded-full bg-[#fef3c7] px-3 py-1 text-xs font-semibold text-[#92400e]">{cartItemCount} item(s)</span>
                        </div>
                        ...
                        {cartItems.length ? (
                            <div className="mt-4 space-y-3">
                                {cartItems.map((item) => {
                                    ...
                                    return (
                                        <div key={id} className="rounded-2xl border border-[#e8dcc2] bg-[#fffaf0] p-3">
                                ...
                                <div className="flex items-center justify-between border-t border-[#ecdcb8] pt-3">
                                    ...
                                    <span className="text-lg font-semibold text-[#0f172a]">{formatMoney(cartSubtotal)}</span>
                                </div>
                            </div>
                        ) : (
                            <p className="mt-4 text-sm text-[#64748b]">Your cart is empty. Add products from the catalog.</p>
                        )}
                    </section>

                    <section className="rounded-3xl border border-[#e4d6b8] bg-white p-5 shadow-[0_20px_50px_rgba(2,6,23,0.08)]">
                        <h2 className="text-xl font-semibold text-[#0f172a]">Checkout</h2>
                        <p className="mt-1 text-xs text-[#64748b]">Payment method: Cash on Delivery</p>

                        {isLoggedIn ? null : (
                            <p className="mt-4 text-sm text-[#475569]">
                                Please{" "}
                                <Link to="/login" className="font-semibold text-[#0f766e] underline">
                                    login
                                </Link>{" "}
                        ...
                        {isLoggedIn && !isUser ? (
                            <p className="mt-4 text-sm text-[#475569]">
                                This storefront checkout is for USER accounts. Continue at your portal:
                                {" "}
                                <button
                                    type="button"
                        ...
                        <form className="mt-4 space-y-3" onSubmit={handlePlaceOrder}>
                            ...
                            >
                                {orderLoading ? "Placing order..." : "Place Cash-on-Delivery Order"}
                            </button>
                        </form>
                    </section>
                </aside>
